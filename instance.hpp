...
namespace utility {
	...
	namespace details {
		...
		public:
			...
			virtual std::uintptr_t offset(std::uintptr_t ptr) const = 0;
			virtual std::uintptr_t getptr() const = 0;
			std::size_t inc() { return counter += 1; }
			std::size_t dec() { return counter -= 1; }
		...
		public:
			...
			std::uintptr_t offset(std::uintptr_t ptr) const override { return ptr - getptr(); }
			std::uintptr_t getptr() const override { return (std::uintptr_t)&object; }
	...
	public:
		...
		T& get() {
			if(!cblock->unique()) {
				...
				auto offset = cblock->offset((std::uintptr_t)object);
				reset();
				cblock = new_cblock;
